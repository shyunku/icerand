...
import { refreshAuthToken, verifyToken } from "components/store/authThunk";
import { useEffect } from "react";
import { toast } from "react-hot-toast";
import { useDispatch, useSelector } from "react-redux";
import { fastInterval } from "static/scripts/Util";
    ...
    const interceptor = axios.interceptors.response.use(
      ...
      async (error) => {
        if (error.request.responseURL) {
          ...
          const reqUrl = new URL(error.config.url);
          const pathname = reqUrl.pathname;
          if (pathname === "/file/upload") {
            return Promise.reject(new AxiosError("upload failed", 403, error.config, error.request, error.response));
          }
        }
